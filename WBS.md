...
- 현재 상태: **26장 제작 완료** (원본 25장 + 골인 화면 1장). 아래 "확인이 필요한 항목"만 남았습니다.

## 0. 월드 구조 — 5문제마다 장소가 바뀝니다

실제 마리오처럼 스테이지가 진행됩니다. HUD의 `WORLD` 표기가 `1-1 → 1-5 → 2-1 → 2-5`로 넘어갑니다.

| 구간 | 스테이지 | 장소(`theme`) | 배경 |
|---|---|---|---|
| 인트로 4장 | — | `dawn` | 아침 하늘, 언덕, 파이프 |
| Q1~Q5 | `1-1` ~ `1-5` | `overworld` | 지상 — 초록 들판, 나무, 파이프 |
| Q6~Q10 | `2-1` ~ `2-5` | `underground` | 지하 동굴 — 벽돌 천장, 파란 블록, 보석 |
| 골인 | — | `castle` | 밤하늘, 성, 깃발, 케이크 |

월드가 바뀌는 순간(Q6 진입)에는 검은 화면에 `WORLD 2-1` 인터미션 카드가 1.7초간 뜹니다.

**핵심 연출 — 정답은 `?` 블록에서 나옵니다.**
정답 슬라이드에 들어가면 `?` 블록과 주인공만 보이고 "정답은?"이 깜빡입니다.
**한 번 더 누르면** 주인공이 점프해 블록을 치고, 코인이 튀어오르며(`+1000`) 정답이 공개됩니다.
진행자가 "정답은…" 하고 뜸을 들이다 공개할 수 있습니다.
...
| `answer` | 정답 (`?`블록 연출) | `no`, `answer`, `blank` |
| `clear` | 골인 화면 | — |

모든 슬라이드 공통 필드: `theme`(장소), `ground`(`grass`/`cave`/`stone`), `ceiling`(지하 천장),
`props`(배경 소품 배열), `hero`(주인공 위치와 걷기 여부), `nav`(목차 이름).

배경 소품은 `SET_OVER`(지상) / `SET_CAVE`(지하) 프리셋을 쓰거나 `px('그림이름','위치 스타일','클래스')`로
직접 지정합니다. 쓸 수 있는 그림은 `ART` 객체에 있습니다 —
`qblock` `brick` `pipe` `hill` `tree` `bush` `flower` `coin` `heart` `stalactite` `gem` `mushroom`
`star` `chest` `cake` `castle` `flagpole` `flag` `baby1~3`.
...
| 키 | 동작 |
|---|---|
| `→` `Space` `Enter` `PageDown` | 다음 — **정답 화면에서는 첫 번째가 "정답 공개"** |
| `←` `PageUp` | 이전 |
...
- **아이폰은 로컬 HTML 파일을 열지 못합니다.** 파일 앱에서 `index.html`을 탭하면 Safari가 아니라
  "빠른 보기"가 열리는데 여기서는 JavaScript가 실행되지 않아 **빈 화면**만 보입니다.
  카카오톡·메일로 파일을 받아 여는 것도 마찬가지입니다. 파일이 잘못된 게 아닙니다.
  → **반드시 웹 주소(GitHub Pages)로 열어야 합니다.** (§6 TODO 참고)
- **가로로 돌려서** 보세요. 세로일 때는 "화면을 가로로 돌려주세요" 안내가 자동으로 뜹니다.
- 넘기기: 화면 오른쪽 탭 / 왼쪽 22% 탭 = 이전 / 좌우 스와이프
...
- [ ] **GitHub Pages 켜기** — 아이폰에서 보려면 웹 주소가 필요합니다. 저장소 웹페이지에서:
      `Settings` → 왼쪽 메뉴 `Pages` → **Source: Deploy from a branch** →
      Branch: **`main`** / 폴더: **`/ (root)`** → `Save`
      1~2분 뒤 아래 주소가 열립니다.
      **https://you4ranghe.github.io/chaewon_1st_birthday_quiz/**
      - ⚠️ 무료 Pages는 **저장소가 public이어야** 합니다. public이면 슬라이드 내용(몸무게·날짜 등)이
        인터넷에 공개됩니다. 감수할지 판단이 필요합니다.
      - 주소가 안 열리면 `Settings → Pages` 화면 상단의 배포 상태를 확인하세요.
