...
└─ _source\                ← 수정 작업은 전부 여기서 합니다
   ...
   ├─ build.ps1            ← 폰트·음악을 심어 index.html을 만드는 스크립트
   ├─ audio\               ← 배경음악 (이름순으로 이어서 반복 재생)
   └─ fonts\
...
`_source\audio` 안의 음원이 **파일 이름순으로 이어서 반복 재생**됩니다. 현재 두 곡이 들어 있습니다.

- 브라우저가 자동재생을 막으므로 **첫 키를 누르는 순간** 시작됩니다. 표지에서 조용한 건 정상입니다.
- 시작 음량은 **30%**입니다. 사회자 목소리가 묻히지 않는 크기로 잡았습니다. `[` `]` 로 조절합니다.
- **정답 팡파레가 울릴 때 배경음악이 자동으로 낮아졌다가** 2.6초 뒤 돌아옵니다.
- 곡을 바꾸려면 `_source\audio` 안의 파일을 교체하고 다시 빌드하면 됩니다. 개수 제한은 없고,
  이름순으로 이어집니다. 파일이 없으면 음악 없이 빌드됩니다.
- 음원이 커질수록 `index.html`도 커집니다. 지금은 두 곡 3MB → 최종 5.75MB입니다.

> ⚠️ 현재 들어 있는 두 곡은 **닌텐도 저작권 음원**이며 저장소가 public이라 그대로 공개됩니다.
> 사용자가 이 위험을 알고 올리기로 결정했습니다. 문제가 되면 음원을 지우고 다시 빌드하면 됩니다.
